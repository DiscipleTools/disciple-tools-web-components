...
if (result && result.node) {
  if (parseInt(result.node) >= 16) {
    console.log("\x1b[32m%s\x1b[0m", `-------******* Good to Go with your Node Version: ${  result.node  } *******-------`);
  } else {
    console.log("\x1b[31m%s\x1b[0m", "-------******* Package installation(npm install) or Project startup command(npm start) failed due to Node Version, Please install and use Node Version >=8 *******-------");
