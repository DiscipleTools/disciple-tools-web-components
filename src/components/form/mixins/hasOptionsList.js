...
export const HasOptionsList = superClass =>
  class extends superClass {
    ...
    static get properties() {
      return {
        ...
        /** Indicates if list should be displayed above the component */
        showAbove: {
          type: Boolean,
          state: true,
        },
    ...
    updated(changedProperties) {
      ...
      if (container) {
        const currentValue =
          container.style.getPropertyValue('--container-width');
      ...
      if (changedProperties && changedProperties.has('open') && this.open) {
        this._checkPosition();
      }
    }
    ...
    _checkPosition() {
      ...
      if (inputGroup) {
        ...
        // Assuming max height of list is 150px (as seen in CSS)
    ...
    get optionListStyles() {
      ...
      if (this.showAbove) {
        styles.bottom = this.containerHeight
          ? `${this.containerHeight}px`
          : '2.5rem';
        styles.top = 'auto';
      } else {
        styles.top = this.containerHeight
          ? `${this.containerHeight}px`
          : '2.5rem';
        styles.bottom = 'auto';
      }
    ...
    _baseRenderOptions() {
      ...
      return repeat(
        this.filteredOptions,
        opt => opt.id,
        (opt, idx) => this._renderOption(opt, idx),
      );
    }
